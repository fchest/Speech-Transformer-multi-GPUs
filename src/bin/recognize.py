...
def recognize(args):
    ...
    with open(args.result_label, 'wb') as f:
        f.write(json.dumps({'utts': new_js}, indent=4,
                           sort_keys=True, ensure_ascii=False).encode('utf_8'))
    # f = open(args.result_label, 'w', encoding="utf-8")
    # f.write(json.dumps({'utts': new_js}, indent=4, sort_keys=True))
    # f.close()
    
    # with open(args.result_label, 'w', encoding="utf-8") as f:
        # c = json.dumps({'utts': new_js}, indent=4, sort_keys=True, ensure_ascii=False)
        # f.write(c)
